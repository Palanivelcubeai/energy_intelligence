...
import { Brain, Lightbulb, Target, Bot, User, Send } from "lucide-react";
...
export default function AIInsights() {
  ...
  useEffect(() => {
    ...
    const fetchEmergencyInsights = async () => {
      try {
        const rt = await apiClient.get("/metrics/realtime", { timeout: 7000 });
    ...
    const fetchInsights = async (isInitial = false) => {
      ...
      if (isInitial) {
        ...
        try {
          const rTop = await apiClient.get("/insights/top", { timeout: 7000 });
          if (!active) return;
      ...
      try {
        const r = await apiClient.get("/insights/all", { timeout: 12000 });
        if (!active) return;
  ...
  const sendChatMessage = async (message: string) => {
    ...
    setChatMessages((prev) => [...prev.slice(-(MAX_CHAT_HISTORY_MESSAGES - 1)), userMessage]);
    setChatInput("");
    setChatLoading(true);
    ...
    try {
      ...
      setChatMessages((prev) => [
        ...prev.slice(-(MAX_CHAT_HISTORY_MESSAGES - 1)),
        {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">LLM-powered optimization recommendations from live plant data</p>
        </div>
      </div>
          ...
          <div className="rounded-xl border border-border/60 bg-card/50 p-4">
            ...
            <div className="mb-3 flex flex-wrap gap-2 items-center">
              ...
              <button
                ...
                className="rounded-md border border-border bg-background px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors"
              >
                Clear chat
              </button>
            </div>

            <div className="max-h-[45vh] md:max-h-72 overflow-y-auto space-y-2 pr-1">
            ...
            <form onSubmit={handleChatSubmit} className="mt-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              <input
                value={chatInput}
              ...
              <button
                ...
                className="h-10 rounded-md bg-primary px-3 text-primary-foreground disabled:opacity-60 sm:w-auto w-full"
              >
